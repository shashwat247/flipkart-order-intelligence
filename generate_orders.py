"""Part 1 Task 1 — generate the exact seeded order dataset.

Run exactly as written:  python3 generate_orders.py

The RNG seed, category/payment lists, probabilities and the return-generating
formula below are fixed by the project brief. Changing any of them (or the
*order* in which the rng is consumed) changes every downstream number, so this
file is deliberately left as a single flat script with no refactoring.
"""
